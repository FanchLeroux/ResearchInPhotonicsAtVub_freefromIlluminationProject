...
function [ r ] = finiteDistanceLambertianSource2rectangularTopHat_XYpolynomials( args )
...
function [r] = BeginApplication(TheApplication, ~)
    ...
    dirc = "D:\moi\vub\researchInPhotonics\zemax\zosApi\lambertianPointSource\rectangularTopHat\";
    ...
    for order = lowerOrder:2:highestOrder % even aspher surface defined by extended polynomial: no order 1 term
        ...
        for currentOrder = 4:2:order % order 2 corresponds to conic constant and only even orders are considered
            ...
            % set even cross terms as variables
